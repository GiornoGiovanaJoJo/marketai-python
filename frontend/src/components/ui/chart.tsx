...
const ChartTooltipContent = React.forwardRef<HTMLDivElement, any>(
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl glass-effect",
          className
        )}
      >
        {tooltipLabel}
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            const itemConfig = item.payload?.config?.[key] || item.payload?.config?.[key] || {}
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  indicator === "dashed" && "border-t border-dashed pt-1.5"
                )}
              >
